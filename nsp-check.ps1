...
if ($associateStorageAccount.Count -eq 0) {
    ...
    foreach ($sa in $associateStorageAccount) {
        ...
        if (Check-PrivateEndpointConnections -storageAccountName $sa.name) {
            Write-Host -ForegroundColor Green "Warning: Storage Account '$($sa.name)' has private endpoint connections. Manual review of private endpoints and the resource firewall is strongly recommended."
        }
    }
