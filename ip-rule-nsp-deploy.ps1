...
.PARAMETER NSP_Type
    An optional parameter to specify the type of NSP profile to create: "SE" (default) for Service Endpoint, "NAT" for Serverless NAT IP CIDR, or "BOTH" for both types. Default is "SE".
.EXAMPLE
...
param(
    ...
    [string]$NSP_Type = "SE", # "SE" for Service Endpoints, "NAT" for Serverless NAT ip CIDR.
...
function collectSEIPs {
    Write-Log "Filtering for Azure inbound NAT traffic..."
    $azureNatIps = $response.prefixes | Where-Object {
        $_.platform -eq "azure" -and $_.type -eq "serviceendpoint"
    }
    
    if (-not $azureNatIps.ipv4Prefixes -or $azureNatIps.ipv4Prefixes.Count -eq 0) {
        Write-Log "No Azure outbound SE IPs found!" "WARNING"
        exit 0
    }
    
    Write-Log "Found $($azureNatIps.ipv4Prefixes.Count) Azure outbound SE IPs"
...
try {
    ...
    switch ($NSP_Type.ToUpper()) {
        "NAT" { $ipsByRegion = collectNATIPs }
        "SE"  { $ipsByRegion = collectSEIPs }
        default { Write-Log "Invalid NSP_Type specified: $NSP_Type. Valid values are 'SE', or 'NAT'" "ERROR"; exit 1 }
    }
    ...
    foreach ($regionGroup in $ipsByRegion) {
        ...
        $ruleName = "databricks-$NSP_Type-$region-rule"
        ...
        if (-not (Get-AzNetworkSecurityPerimeterAccessRule -Name $ruleName -ProfileName $profileName -ResourceGroupName $Resource_Group -SecurityPerimeterName $NSP_Name -ErrorAction SilentlyContinue)) {
            ...
            Remove-AzNetworkSecurityPerimeterAccessRule -Name $ruleName -ProfileName $profileName -ResourceGroupName $Resource_Group -SecurityPerimeterName $NSP_Name | Out-Null
        }
        # Create new rule with updated CIDRs
        
        # Update profile with rules
        New-AzNetworkSecurityPerimeterAccessRule -Name $ruleName -ProfileName $profileName -ResourceGroupName $Resource_Group -SecurityPerimeterName $NSP_Name -AddressPrefix @($cidrs) -Direction 'Inbound' | Out-Null
